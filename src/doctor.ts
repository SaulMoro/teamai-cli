 ...
 */
async function buildEnvDeliveryCheck(ctx: DoctorContext): Promise<Check[]> {
...
async function envDeliveryProblems(ctx: DoctorContext): Promise<string[]> {
  ...
  const declared = (await new EnvHandler().parseEnvYaml(envYamlPath)).variables;
  ...
  const profilePath = teamConfig?.sharing?.env?.shellProfilePath
    ?? path.join(getUserHome(), shell.includes('zsh') ? '.zshrc' : '.bashrc');
