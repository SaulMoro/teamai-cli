...
- If the team repo cannot be refreshed at the start of a push, `--project` stops instead of placing by a possibly stale `manifest/projects.yaml`; so does a new resource that would resolve from `manifest/roles.yaml`. Fix the pull and retry, or name the namespace with `--role <ns>`
